...
GitHub Actions #815でJSONDecodeError（Expecting ',' delimiter）が発生。APIキー不備ではなく応答JSONの解析失敗だった。主処理にresponse_schema、必須本文・配列・出典URL・終了理由の検証、最大3回の再試行を追加。失敗時のダミー公開を廃止し、保存前に例外終了する。Actionsの失敗サマリーも追加。メール等の配信はユーザーのGitHub通知設定に依存する。

`python -m unittest test_editorial_failure -v` でAPI通信なしに6項目を確認。公開中の代替文は9月14日07:48の正常記事に復元（掲載日は維持、失敗日の保存JSONも調査用に保持）。この変更は後述の旧「ダミーに落ちて成功扱い」という既知課題を主処理について解消する。
