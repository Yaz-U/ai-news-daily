...
.issue-dateline{border-left:3px solid var(--accent);padding:4px 0 4px 18px;margin:0 0 28px}.issue-dateline .date-label{display:block;font-size:12px;color:var(--accent);font-weight:600;margin-bottom:6px}.issue-dateline time{display:block;font-size:clamp(22px,3vw,28px);font-weight:700;line-height:1.5;font-variant-numeric:tabular-nums;letter-spacing:.02em}.issue-dateline .date-update{display:block;font-size:13px;color:var(--ink);margin-top:8px}.issue time{font-size:14px;color:var(--ink);font-weight:600}
...
def issue_date(timestamp):
    try:
        ...
        return f"{parsed.year}年{parsed.month}月{parsed.day}日（{weekday}）", parsed.strftime("%H:%M")
...
def render_issue(data, prefix=""):
    ...
    content = f'''<div class="intro"><div class="issue-dateline"><span class="date-label">掲載日 · DAILY BRIEF</span><time datetime="{esc(data.get('timestamp', ''))}">{date}</time><span class="date-update">{time + ' 更新（日本時間）' if time else '更新時刻未記録'}</span></div><h1>{esc(title)}</h1><div class="standfirst">{paragraphs(summary.get("thesis"))}</div><div class="meta"><span>AI News Daily 編集部</span><span>読了目安 {max(1, round(chars / 550))}分</span></div></div>
